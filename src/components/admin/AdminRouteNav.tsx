  ...
  return (
    <>
      <div className="lg:hidden">
        <button
          className="inline-flex items-center gap-2 rounded-lg border border-white/15 bg-white/[0.07] px-3 py-2 text-sm font-semibold text-slate-100 transition hover:bg-white/[0.12]"
          type="button"
      ...
      {drawerOpen ? (
        <div className="fixed inset-0 z-50 lg:hidden">
          ...
          />
          <div className="relative h-full w-[min(21rem,90vw)] border-r border-white/10 bg-slate-950 p-4 shadow-2xl">
            <div className="mb-4 flex items-center justify-between">
              <p className="text-sm font-semibold text-slate-200">Admin navigation</p>
              <button
                className="rounded-md border border-white/15 p-2 text-slate-200 transition hover:bg-white/10"
                type="button"
                aria-label="Close menu"
                onClick={() => setDrawerOpen(false)}
              >
      ...
      <div className="lg:hidden">
        <Badge className="border-white/20 bg-white/10 text-slate-100">
  ...
  return (
    <div className="space-y-4 lg:sticky lg:top-24">
      <div className="rounded-xl border border-white/10 bg-white/[0.04] p-4">
        ...
        <p className="mt-2 text-lg font-bold text-white">Operations console</p>
        <p className="mt-1 text-sm text-slate-300">Route-level tools grouped for daily operations.</p>
      </div>
      <nav className="rounded-xl border border-white/10 bg-white/[0.04] p-2">
        {adminLinks.map((link) => {
      ...
      <div className="rounded-xl border border-white/10 bg-white/[0.04] p-4">
        <p className="text-xs font-semibold uppercase tracking-[0.18em] text-slate-400">Live pulse</p>
  ...
  return (
    <div className="flex items-center justify-between gap-3 rounded-lg border border-white/10 bg-white/[0.03] px-3 py-2">
      <span className="flex items-center gap-2 text-slate-300">
        {Icon ? <Icon className="h-3.5 w-3.5 text-emerald-300" /> : null}
        {label}
      </span>
