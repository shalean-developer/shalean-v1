...
create table if not exists public.services (
  ...
  min_hours numeric(5, 2) not null default 3,
...
begin
  if exists (
    ...
    select 1 from pg_type where typname = 'booking_cleaner_role'
  ) then
  ...
  if exists (
    ...
    select 1 from pg_type where typname = 'booking_cleaner_status'
  ) then
...
create table if not exists public.payments (
  ...
  status public.v1_payment_status not null default 'pending',
