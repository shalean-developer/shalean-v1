-- Ensure admin-managed pricing tables are created after the latest existing
-- production migrations. This intentionally repeats the earlier pricing DDL
-- idempotently because 20260528232957 can sort before already-applied 202607
...
begin
  if not exists (
    select 1
    from pg_constraint
    where conname = 'services_base_price_cents_nonnegative'
...
insert into public.service_addons
  (service_slug, key, label, description, price_cents, duration_minutes, workload_weight, active, sort_order)
values
  ('regular-cleaning', 'laundryIroning', 'Laundry & Ironing', 'Laundry load support for wash, hang, fold, or rotate, plus light household ironing.', 9000, 60, 1.20, true, 50)
on conflict (service_slug, key) do update set
